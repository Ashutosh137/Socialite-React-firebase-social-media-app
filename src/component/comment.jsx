...
export default function Comment({ currentcomment, toggle = () => { }, setpost }) {
    ...
    useEffect(() => {
        const data = async () => {
            if (comment && auth.currentUser) {
                setpost(prev => ({
                    ...prev, 'comments': prev?.comments.map((currcomment) => {
    ...
    const delcomment=()=>{
        setpost(pre=>({...pre, 'comments': pre?.comments.filter(curr=>curr.commentid!== comment?.commentid)}))
    }


    return <div className=" w-full ">
        {!delet && <div className='capitalize flex w-full  sm:space-x-2 '>
            ...
            <div className="flex w-full space-y-1 sm:mx-3 flex-col">
                <div className='flex relative text-sm'>
                    ...
                    <label className='text-gray-500 text-xs  sm:text-sm ' >{Time(comment.postedat?.toJSON().seconds) || <Skeleton animation="wave" sx={{ bgcolor: 'grey.900' }} variant="text" width={200} />}</label>
                    <label className='ml-auto mx-2' onClick={() => { active === 'menu' ? setactive('') : setactive('menu') }}><MoreVertIcon /></label>
                    {active === 'menu' && <div className='absolute top-5 right-3 sm:right-8 px-4 text-sm bg-black sm:-my-16 -my-5 py-5 p-3 rounded-xl shadow-sm shadow-white flex flex-col space-y-2  '>
                        <button className='w-40 p-1 rounded-full text-white hover:bg-gray-950 ' onClick={() => {
                            navigate(`/profile/${commentby?.username} `)
                            toggle()
                        }}>View profile</button>
                        <button className='w-40 p-1 rounded-full text-red-500 hover:bg-gray-950 ' onClick={() => {
                            setactive('report')

                        }}>Report</button>
                        {(userdata?.username === commentby?.username || post?.postedby === userdata?.uid) && <button className='w-40 p-1 rounded-full text-red-500 hover:bg-gray-950 ' onClick={() => {
                        setdelet(true)
                        delcomment()
                        }}>Delete</button>}
                    </div>}
                ...
                <div className="flex flex-col my-2">
                    ...
                    <div className="flex mr-auto my-2 text-gray-400 space-x-1  " onClick={() => {
                        auth.currentUser && comment?.likes.includes(userdata?.uid) ? setcomment(prev => ({ ...prev, 'likes': prev.likes.filter((e) => e !== userdata?.uid) })) : setcomment(prev => ({ ...prev, 'likes': [...prev.likes, userdata?.uid] }))
                    }
                    }>
                        <label className=' text-sm my-auto'  >{comment.likes.length}</label>
                        <i className='hover:text-red-900'>{comment?.likes.includes(userdata?.uid) ? <FavoriteIcon style={{ 'color': '#CF000F' }} /> : <FavoriteBorderIcon />}</i>
                    </div>
                </div>
                {active === 'report' && <Popupitem closefunction={() => { setactive('') }} >
                    <div className="max-w-sm m-auto mb-10">
                        <p className='text-xl text-center my-7 capitalize '>report this comment</p>
        ...
        {delet && <p className='text-center text-sm capitalize text-zinc-600 '>
            comment deleted
        </p>}
    </div>
}
