      ...
      <header className='p-2 hidden sm:block w-20 md:w-96   '>
        <div className="fixed flex flex-col h-screen md:p-4 ">
          ...
          <nav className="flex my-auto align-middle text-xl justify-center capitalize flex-col">
            ...
            <Link to={`${userdata?.username ? `/lists` : '/login'}`}><span className='flex hover:bg-gray-700  p-3 px-5 m-auto rounded-full justify-center md:justify-start'><SearchIcon /><label className='md:block hidden mx-2' >explore</label></span></Link>
            <Link to={`${userdata?.username ? `/lists` : '/login'}`}><span className='flex hover:bg-gray-700  p-3 px-5 m-auto rounded-full justify-center md:justify-start'><NotificationsIcon /><label className='md:block hidden mx-2' >Notifications</label></span></Link>
            <Link to={`${userdata?.username ? `/lists` : '/login'}`} ><span className='flex hover:bg-gray-700  p-2 px-5 m-auto rounded-full justify-center md:justify-start'><MessageIcon /><label className='md:block hidden mx-2' >Message</label></span></Link>
            <Link to={`${userdata?.username ? `/lists` : '/login'}`} ><span className='flex hover:bg-gray-700  p-3 px-5 m-auto rounded-full justify-center md:justify-start'><PeopleIcon /><label className='md:block hidden mx-2' >community</label></span></Link>
            <Link to={`${userdata?.username ? `/lists` : '/login'}`} ><span className='flex hover:bg-gray-700  p-3 px-5 m-auto rounded-full justify-center md:justify-start'><BookmarksIcon /><label className='md:block mx-2 hidden' >lists</label></span></Link>
            <Link to={`${userdata?.username ? `/profile/${userdata?.username}` : '/login'}`} ><span className='flex hover:bg-gray-700  p-3 px-5 m-auto rounded-full justify-center md:justify-start'><PersonIcon /><label className='md:block mx-2 hidden' >proflie</label></span></Link>

            <Link to={`${userdata?.username ? `/lists` : '/login'}`} ><span className='flex hover:bg-gray-700  p-3 md:px-5 m-auto rounded-full justify-center md:justify-start'><SettingsIcon /><label className='md:block hidden mx-2' >settings </label></span></Link>
            <button onClick={() => {
              setpost(true)
